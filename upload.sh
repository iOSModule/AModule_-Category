...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' A_Category.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" A_Category.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' A_Category.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" A_Category.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' A_Category.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" A_Category.podspec
...
VersionLineNumber=`grep -nE 's.version.*=' A_Category.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" A_Category.podspec
...
pod repo push ${Repo} A_Category.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
